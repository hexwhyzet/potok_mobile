...
import 'package:potok/old/config.dart';
...
class _TrendingProfilesState extends State<TrendingProfiles> {
  ...
  void initState() {
    ...
    this.scrollController.addListener(() {
      if (storage.hasMore && !storage.isProfileLoading && scrollController.position.extentAfter < 100) {
  ...
  @override
  Widget build(BuildContext context) {
    if (storage.size() == 0 && storage.isProfileLoading == true) {
      return Center(
        child: StyledLoadingIndicator(
          color: theme.colors.secondaryColor,
        ),
